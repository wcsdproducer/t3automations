...
export default async function BlogPostPage({ params }: PageProps) {
  const { userId, slug } = await params;
  if (!userId || !slug) return notFound();
  ...
  try {
    ...
    console.error('[blog-post] Profile fetch error:', error);
    return notFound();
  }
  ...
  try {
    ...
    console.error('[blog-post] Blog post fetch error:', error);
  }
    ...
    <div className="bg-slate-50 min-h-screen text-slate-800 flex flex-col font-sans">
      ...
      <header className="sticky top-0 z-50 py-4 px-6 md:px-12 flex justify-between items-center border-b bg-white/80 backdrop-blur-sm">
        ...
        <nav className="hidden md:flex gap-6 items-center">
          ...
          <Link href={`/pages/${userId}/blog`} className="text-sm font-medium hover:text-blue-600 transition-colors">Blog</Link>
        </nav>
        {phone && (
      ...
      <main className="flex-grow container mx-auto px-6 py-12 max-w-5xl">
        <div className="mb-6">
          <Link href={`/pages/${userId}/blog`} className="flex items-center gap-2 text-sm text-slate-500 hover:text-blue-600 transition-colors">
                ...
                <div className="space-y-4">
                  {recentPosts.map((rp) => (
                    <div key={rp.id} className="group">
                      <Link href={`/pages/${userId}/blog/${rp.slug}`}>
