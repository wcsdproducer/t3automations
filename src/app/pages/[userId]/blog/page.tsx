...
export default async function BlogIndexPage({
  ...
  try {
    ...
    console.error('[blog-index] Profile fetch error:', error);
    return notFound();
  }
  ...
  try {
    ...
    console.error('[blog-index] Blogs fetch error:', error);
  }
    ...
    <div className="bg-slate-50 min-h-screen text-slate-800 flex flex-col font-sans">
      ...
      <header className="sticky top-0 z-50 py-4 px-6 md:px-12 flex justify-between items-center border-b bg-white/80 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          <Link href={`/pages/${userId}`} className="text-2xl font-bold text-blue-600 hover:opacity-85 transition-opacity">
            {companyName}
          </Link>
        </div>
        <nav className="hidden md:flex gap-6 items-center">
          <Link href={`/pages/${userId}`} className="text-sm font-medium hover:text-blue-600 transition-colors">Home</Link>
          <Link href={`/pages/${userId}#services`} className="text-sm font-medium hover:text-blue-600 transition-colors">Services</Link>
          <Link href={`/pages/${userId}#about`} className="text-sm font-medium hover:text-blue-600 transition-colors">About Us</Link>
          <Link href={`/pages/${userId}/blog`} className="text-sm font-semibold text-blue-600">Blog</Link>
        </nav>
        {phone && (
      ...
      <main className="flex-grow container mx-auto px-6 py-12 max-w-6xl">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <Link href={`/pages/${userId}`} className="flex items-center gap-2 text-sm text-slate-500 hover:text-blue-600 transition-colors mb-2">
        ...
        {blogs.length === 0 ? (
          <div className="bg-white border rounded-2xl p-12 text-center max-w-md mx-auto shadow-sm">
            ...
            <Link href={`/pages/${userId}`} className="inline-block mt-6 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">
              Return Home
            </Link>
          </div>
        ) : (
              ...
              <article key={post.id} className="bg-white border rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col h-full">
                ...
                <div className="p-6 flex-grow flex flex-col justify-between">
                  <div>
                    ...
                    <Link href={`/pages/${userId}/blog/${post.slug}`} className="group">
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-100">
                    <Link href={`/pages/${userId}/blog/${post.slug}`} className="text-blue-600 hover:text-blue-700 text-sm font-bold inline-flex items-center gap-1.5 transition-colors">
                      Read Article &rarr;
                    </Link>
                  </div>
                </div>
      ...
      <footer className="py-8 px-6 text-center text-slate-500 border-t bg-white">
        ...
        <div className="mt-4 flex justify-center gap-4 text-sm">
          <a href={`/api/legal/privacy?userId=${userId}`} target="_blank" className="hover:underline">Privacy Policy</a>
          <a href={`/api/legal/tos?userId=${userId}`} target="_blank" className="hover:underline">Terms of Service</a>
        </div>
      </footer>
    </div>
  );
