...
import { Wrench, ShieldCheck, Smile, Star, Phone } from 'lucide-react';
...
export function Template1Content({
  businessProfileId,
    ...
    <div className="bg-background text-foreground">
      ...
      <header className="sticky top-0 z-50 py-4 px-6 md:px-12 flex justify-between items-center border-b bg-background/80 backdrop-blur-sm">
        ...
        <nav className="hidden md:flex gap-6 items-center">
          ...
          <a href={blogLink} className="text-sm font-medium hover:text-primary transition-colors">Blog</a>
        </nav>
            ...
            <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-8">
              {content.services.items.map((item: any, index: number) => (
                <div key={index} className="p-6 border rounded-lg flex flex-col items-center transition-all duration-300 hover:shadow-xl hover:-translate-y-2">
                  {serviceIcons[item.title] || <Wrench className="h-10 w-10 mx-auto text-primary" />}
            ...
            <div>
              ...
              <ul className="mt-6 space-y-2">
                {content.about.points?.map((point: string, i: number) => (
                  <li key={i} className="flex items-center gap-3"><Wrench className="h-5 w-5 text-primary" />{point}</li>
                ))}
              </ul>
            </div>
            {aboutImage && (
        ...
        <section id="contact" className="bg-muted py-16 md:py-24 px-6">
          <div className="container mx-auto max-w-2xl text-center">
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="mt-8 space-y-4 text-left">
              ...
              <div>
                <Textarea placeholder="Tell us about your project..." {...form.register('notes')} />
