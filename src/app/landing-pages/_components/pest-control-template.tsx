...
export function PestControlTemplate({
  ...
  return (
    <div className="bg-[#020617] text-slate-100 font-sans antialiased min-h-screen">
      <AeoSchema
      ...
      <div className="bg-emerald-950 text-emerald-100 py-2.5 px-6 text-center text-xs font-semibold flex justify-center items-center gap-4 border-b border-emerald-900/30">
      ...
      {/* Header */}
      <header className="sticky top-0 z-50 py-4 px-6 md:px-12 flex justify-between items-center border-b border-slate-900 bg-[#020617]/95 backdrop-blur-md">
        ...
        <div className="flex items-center gap-6">
          <a href={blogLink} className="text-sm font-semibold text-slate-300 hover:text-emerald-400 transition-colors">Blog</a>
          <a 
            href={`tel:${phone}`} 
      ...
      <main>
        {/* Hero Section */}
        <section className="relative py-16 lg:py-24 px-6 md:px-12 bg-gradient-to-br from-[#020617] via-[#020617] to-emerald-950/20 overflow-hidden border-b border-slate-900">
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            <div className="lg:col-span-7 space-y-6 text-center lg:text-left">
              <Badge className="bg-emerald-500/20 hover:bg-emerald-500/20 text-emerald-300 border-none font-bold text-xs uppercase px-3 py-1 tracking-wider">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4 max-w-lg">
                {(content.about?.points || ["Child & Pet Safe Products", "Licensed & Insured Techs", "100% Pest-Free Guarantee", "Free Structural Inspections"]).map((feat: string, idx: number) => (
                  <div key={idx} className="flex items-center gap-2.5 text-sm font-semibold text-slate-350">
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-4 justify-center lg:justify-start">
                ...
                <Button asChild variant="outline" size="lg" className="border-slate-800 hover:bg-slate-900/60 font-bold h-12 px-8">
                  <a href={`tel:${phone}`} className="text-slate-300 flex items-center gap-2">
            ...
            <div className="lg:col-span-5 relative w-full aspect-[4/3] sm:aspect-video lg:aspect-square rounded-2xl overflow-hidden shadow-2xl shadow-emerald-950/20 border border-slate-900 bg-slate-950">
              {content.images?.hero && content.images.hero.length > 0 ? (
                ...
                <div className="flex h-full items-center justify-center bg-slate-900">
                  <Bug className="h-16 w-16 text-emerald-500/50" />
                </div>
              )}
            </div>
          </div>
        ...
        <section id="services" className="py-16 md:py-24 px-6 md:px-12 bg-[#020617]">
          <div className="max-w-7xl mx-auto space-y-12">
            <div className="text-center space-y-4">
              <Badge className="bg-emerald-500/20 hover:bg-emerald-500/20 text-emerald-300 border-none font-bold text-xs uppercase px-3 py-1">
                🛡️ Complete Defense
              </Badge>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {(content.services?.items || pestServices).map((srv: any, idx: number) => (
                <div key={idx} className="p-6 bg-slate-900/40 border border-slate-900 rounded-xl hover:shadow-lg hover:border-slate-800 transition-all duration-300 space-y-3">
                  <div className="h-10 w-10 bg-emerald-500/20 rounded-lg flex items-center justify-center">
                    {srv.icon || <Bug className="text-emerald-500 h-5 w-5" />}
                  </div>
                  <h3 className="text-lg font-bold text-white">{srv.title}</h3>
                  <p className="text-sm text-slate-400 leading-relaxed">{srv.desc || srv.description}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="bg-emerald-950/80 text-white py-12 px-6 text-center border-t border-emerald-900/60 backdrop-blur-sm">
          <div className="max-w-4xl mx-auto space-y-6">
            ...
            <p className="text-emerald-250 text-sm md:text-base max-w-2xl mx-auto">
              Call us immediately to reserve your slot and have a certified exterminator inspect your home today.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
              <a href={`tel:${phone}`} className="bg-emerald-500 hover:bg-emerald-400 text-white font-bold h-12 px-8 rounded-md inline-flex items-center justify-center gap-2 text-sm transition-colors">
                <Phone className="h-4 w-4" /> Call {phone}
              </a>
              {bookingUrl && (
                <a href={bookingUrl} target="_blank" rel="noopener noreferrer" className="bg-transparent hover:bg-white/10 text-white font-bold h-12 px-8 rounded-md border border-white/25 inline-flex items-center justify-center text-sm transition-colors">
                  Book Online Direct
                </a>
              )}
            </div>
        ...
        {/* About Section */}
        <section id="about" className="py-16 md:py-24 px-6 md:px-12 bg-[#020617] border-b border-slate-900">
          <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            {/* Left side: Photo */}
            <div className="relative w-full aspect-[4/3] rounded-2xl overflow-hidden shadow-xl border border-slate-900 bg-slate-950">
              {content.images?.about?.imageUrl ? (
                ...
                <div className="flex h-full items-center justify-center bg-slate-900">
                  <ShieldCheck className="h-16 w-16 text-emerald-500/50" />
                </div>
              )}
            </div>
            ...
            <div className="space-y-6">
              ...
              <p className="text-slate-405 text-base leading-relaxed">
              ...
              <div className="space-y-3.5">
                {(content.about?.points || ["Licensed & Screened Technicians", "Family & Pet Friendly Safe Products", "Targeted Perimeter Barrier Defense Plans"]).map((point: string, idx: number) => (
                  <div key={idx} className="flex items-center gap-3 text-sm font-semibold text-slate-300">
        ...
        {/* Gallery Section */}
        <section id="gallery" className="py-16 md:py-24 px-6 md:px-12 bg-slate-900/20 border-b border-slate-900">
          <div className="max-w-7xl mx-auto space-y-12">
            <div className="text-center space-y-4">
              <Badge className="bg-emerald-500/20 hover:bg-emerald-500/20 text-emerald-300 border-none font-bold text-xs uppercase px-3 py-1">
                📸 Service Showcase
              </Badge>
              <h2 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight">Eco-Friendly Barrier Protection in Action</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {content.images?.gallery && content.images.gallery.length > 0 ? (
                content.images.gallery.map((img: any, idx: number) => (
                  <div key={idx} className="relative aspect-square rounded-xl overflow-hidden shadow-md border border-slate-900 bg-slate-950 group">
                ...
                [1, 2, 3, 4].map(i => (
                  <div key={i} className="aspect-square rounded-xl bg-slate-900 flex items-center justify-center">
                    <Bug className="h-8 w-8 text-slate-500" />
                  </div>
                ))
              )}
            </div>
        ...
        {/* Testimonials */}
        <section id="reviews" className="py-16 md:py-24 px-6 md:px-12 bg-[#020617]">
          <div className="max-w-7xl mx-auto space-y-12">
            <div className="text-center space-y-4">
              <Badge className="bg-emerald-500/20 hover:bg-emerald-500/20 text-emerald-300 border-none font-bold text-xs uppercase px-3 py-1">
                ⭐ Rated 5 Stars
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {uniqueReviews.slice(0, 3).map((review: any, idx: number) => (
                <div key={idx} className="p-6 bg-slate-900/40 border border-slate-900 rounded-xl relative space-y-4 flex flex-col justify-between hover:border-slate-800 transition-colors">
                  ...
                  <span className="text-xs font-bold text-emerald-400 block">{review.author}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* FAQs */}
        <section id="faq" className="py-16 md:py-24 px-6 md:px-12 bg-slate-900/20 border-t border-slate-900">
          <div className="max-w-4xl mx-auto space-y-12">
            ...
            <Accordion type="single" collapsible className="w-full space-y-4">
              {(content.faqs || localFaqs).map((faq: any, idx: number) => (
                <AccordionItem key={idx} value={`item-${idx}`} className="bg-slate-900/50 border border-slate-900 rounded-lg px-6 py-1">
                  <AccordionTrigger className="text-sm md:text-base font-bold text-white hover:text-emerald-400 transition-colors text-left">
                    {faq.question}
                  </AccordionTrigger>
        ...
        {/* Contact/Quote Form */}
        <section id="quote-form" className="py-16 md:py-24 px-6 md:px-12 bg-[#020617]">
          <div className="max-w-3xl mx-auto space-y-8 bg-slate-900/30 border border-slate-900 p-8 md:p-12 rounded-2xl shadow-sm">
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <Label htmlFor="name" className="text-xs font-bold text-slate-300 uppercase">Your Name</Label>
                  <Input id="name" placeholder="John Doe" className="bg-[#020617] border-slate-800 text-white focus-visible:ring-emerald-500" {...form.register('name')} />
                  {form.formState.errors.name && <p className="text-xs text-red-500 font-semibold">{form.formState.errors.name.message}</p>}
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="phone" className="text-xs font-bold text-slate-300 uppercase">Phone Number</Label>
                  <Input 
                    id="phone" 
                    placeholder="(555) 555-5555" 
                    className="bg-[#020617] border-slate-800 text-white focus-visible:ring-emerald-500" 
                    {...form.register('phone')} 
                    onChange={(e) => {
              ...
              <div className="space-y-2">
                <Label htmlFor="email" className="text-xs font-bold text-slate-300 uppercase">Email Address</Label>
                <Input id="email" type="email" placeholder="john@example.com" className="bg-[#020617] border-slate-800 text-white focus-visible:ring-emerald-500" {...form.register('email')} />
                {form.formState.errors.email && <p className="text-xs text-red-500 font-semibold">{form.formState.errors.email.message}</p>}
              </div>

              <div className="space-y-2">
                <Label htmlFor="notes" className="text-xs font-bold text-slate-300 uppercase">Tell us about your pest issue (Optional)</Label>
                <Textarea id="notes" placeholder="Describe the pests, areas affected, or any special requests..." className="bg-[#020617] border-slate-800 text-white focus-visible:ring-emerald-500 min-h-[100px]" {...form.register('notes')} />
              </div>
              ...
              <div className="space-y-4 pt-2">
                <div className="flex items-start space-x-3">
                  <Checkbox 
                    ...
                    className="border-slate-800 data-[state=checked]:bg-emerald-600 data-[state=checked]:border-emerald-600 mt-1" 
                  />
                  <div className="grid gap-1.5 leading-none">
                    <label htmlFor="consent" className="text-xs text-slate-400 font-medium leading-normal">
      ...
      {/* Footer */}
      <footer className="bg-slate-950 text-slate-450 py-12 px-6 md:px-12 border-t border-slate-900">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-2">
            <Bug className="h-5 w-5 text-emerald-500" />
            <span className="text-white font-bold">{companyName}</span>
          </div>
          <p className="text-xs text-slate-500">© {new Date().getFullYear()} {companyName}. All rights reserved. Child-and-pet-safe local pest treatments.</p>
        </div>
      </footer>
