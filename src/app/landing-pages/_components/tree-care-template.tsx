...
import { 
  ...
  Calendar, ArrowRight, TreePine, Leaf, Scissors, Trash2, 
  AlertTriangle, Eye, Sparkles, MapPin, CheckCircle2 
} from 'lucide-react';
...
function ServiceIcon({ name, className }: { name?: string; className?: string }) {
  // Map standard icons to nice arbor/forestry icons
  if (name === 'Scissors' || name === 'scissors') return <Scissors className={className} />;
  if (name === 'Leaf' || name === 'leaf') return <Leaf className={className} />;
  if (name === 'Trash' || name === 'trash' || name === 'Trash2') return <Trash2 className={className} />;
  if (name === 'AlertTriangle' || name === 'alert') return <AlertTriangle className={className} />;
  if (name === 'Award') return <Award className={className} />;
  if (name === 'Sparkles') return <Sparkles className={className} />;
  
  const Icon = (LucideIcons as any)[name || 'TreePine'] || TreePine;
  return <Icon className={className} />;
}

export function TreeCareTemplate({
  businessProfileId,
  ...
  useEffect(() => {
    const staticContent = getContentForService("Tree Services");
    if (websiteConfig) {
      const mergedReviews = [
  ...
  const heroContent = (
    <div className="relative z-10 p-6 max-w-4xl text-left">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-8 max-w-lg">
        {["Licensed, Bonded & Insured", "ISA Certified Arborists on Staff", "24/7 Emergency Storm Dispatch", "Zero-Property-Damage Guarantee"].map((feat, idx) => (
      ...
      <div className="flex flex-wrap gap-4 mt-10">
        <a href="#contact">
          <Button size="lg" className="bg-emerald-700 hover:bg-emerald-800 text-white font-bold h-14 px-8 rounded-xl transition-all hover:scale-105 shadow-lg shadow-emerald-700/20">
            {content.hero.ctaText || "REQUEST FREE ESTIMATE"}
          </Button>
        </a>
        <a href={`tel:${phone}`}>
          <Button size="lg" variant="outline" className="border-slate-300 hover:bg-slate-50 text-slate-800 font-bold h-14 px-8 rounded-xl flex items-center gap-2 bg-white">
            <Phone className="h-5 w-5 text-emerald-700" /> Call {phone}
          </Button>
        </a>
      </div>
    </div>
  ...
  const renderHero = () => {
    return (
      <section className="relative min-h-[90vh] flex items-center bg-gradient-to-br from-emerald-50/50 via-white to-stone-50 py-20 px-6 overflow-hidden">
        {/* Subtle background decoration */}
        <div className="absolute right-0 top-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-emerald-50 rounded-full blur-3xl -z-10 opacity-70" />
        <div className="container mx-auto grid lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-5 relative aspect-[4/3] md:aspect-square w-full rounded-[32px] overflow-hidden shadow-2xl border-4 border-white">
            {heroImages && heroImages[0] ? (
              <Image src={heroImages[0].imageUrl} alt="Arborist trimming tree" fill className="object-cover" priority />
            ) : (
              <div className="absolute inset-0 bg-emerald-100 flex items-center justify-center"><TreePine className="h-20 w-20 text-emerald-700" /></div>
            )}
          </div>
        </div>
      </section>
  ...
  return (
    <div className="bg-white text-slate-900 font-sans antialiased">
      ...
      <div className="bg-emerald-950 text-emerald-100 py-2.5 px-6 text-center text-xs font-semibold flex justify-center items-center gap-4">
      ...
      <header className="sticky top-0 z-50 py-4 px-6 md:px-12 flex justify-between items-center border-b border-slate-100 bg-white/95 backdrop-blur-md">
        ...
        <nav className="hidden md:flex gap-8 items-center text-sm font-bold text-slate-600">
          ...
          <a href="#process" className="hover:text-emerald-700 transition-colors">How We Work</a>
          ...
          <a href={blogLink} className="hover:text-emerald-700 transition-colors">Blog</a>
        </nav>
        <a href={`tel:${phone}`} className="hidden sm:flex items-center gap-2 bg-emerald-50 text-emerald-800 font-bold px-4 py-2 rounded-xl border border-emerald-100 text-sm hover:bg-emerald-100 transition-all">
      ...
      <main>
        ...
        <section className="bg-slate-50 border-y border-slate-100 py-12">
          <div className="container mx-auto px-6 max-w-5xl">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
              {[
                { icon: ShieldCheck, title: "100% Fully Insured", desc: "Equipped with general liability & workers' comp for maximum safety." },
                { icon: Award, title: "ISA Certified Arborists", desc: "Expert tree health diagnostics, trimming, and safe rigging." },
                { icon: ThumbsUp, title: "Satisfaction Guarantee", desc: "We complete every tree job cleanly with a zero-damage warranty." }
              ].map((item, idx) => (
        ...
        <section id="services" className="py-20 md:py-28 px-6 bg-white">
          <div className="container mx-auto text-center max-w-5xl">
            <span className="text-xs font-bold uppercase tracking-widest text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100">Professional Solutions</span>
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 mt-5 tracking-tight">
              {content.services.title}
            </h2>
            <div className="w-16 h-1 mx-auto mt-4 rounded bg-emerald-600" />
                ...
                <div key={i} className="bg-slate-50 p-8 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-between hover:shadow-xl hover:-translate-y-2 transition-all duration-300 text-left group">
                  <div>
                    <div className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center mb-6 transition-transform duration-300 group-hover:scale-110">
                      <ServiceIcon name={item.iconName || item.icon} className="h-6 w-6 text-emerald-700" />
                    </div>
                    <h3 className="text-xl font-bold text-slate-900">{item.title}</h3>
                    <p className="mt-3 text-slate-500 text-sm leading-relaxed">{item.description}</p>
                  </div>
                  <a href="#contact" className="mt-8 inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-emerald-800 hover:underline">
        ...
        {content.process && (
          <section id="process" className="py-20 md:py-28 px-6 bg-slate-50 border-y border-slate-100">
            <div className="container mx-auto text-center max-w-5xl">
              <span className="text-xs font-bold uppercase tracking-widest text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100">Strict Safety Standards</span>
              <h2 className="text-3xl md:text-5xl font-black text-slate-900 mt-5 tracking-tight">
                {content.process.title || "How It Works"}
              </h2>
              <div className="w-16 h-1 mx-auto mt-4 rounded bg-emerald-600" />
              <p className="text-slate-500 mt-4 max-w-xl mx-auto text-base">
                {content.process.subtitle || "How we ensure your property remains safe and pristine."}
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-16 relative">
                {content.process.steps?.map((step: any, i: number) => (
                  <div key={i} className="bg-white p-8 rounded-3xl border border-slate-100 relative hover:shadow-lg transition-all text-center group">
                    <div className="absolute -top-6 left-1/2 -translate-x-1/2 w-12 h-12 rounded-full flex items-center justify-center font-extrabold text-sm text-white shadow-md transition-all duration-300 group-hover:scale-110 bg-emerald-700">
                      0{step.number || (i + 1)}
                    </div>
        ...
        <section className="bg-white py-20 md:py-28 px-6">
          <div className="container mx-auto text-center max-w-5xl">
            <span className="text-xs font-bold uppercase tracking-widest text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100">The Arborist Standard</span>
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 mt-5 tracking-tight">
              Why Choose Our Tree Care
            </h2>
            <div className="w-16 h-1 mx-auto mt-4 rounded bg-emerald-600" />
            <p className="text-slate-500 mt-4 max-w-xl mx-auto text-base">
              We protect your landscape, homes, and structures using industry-best safe rigging and arborist methods.
            </p>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mt-16">
              {[
                { icon: "ShieldCheck", title: "Comprehensive Insurance", desc: "General liability & workers' comp coverage to guarantee homeowner security." },
                { icon: "TreePine", title: "Arboriculture Specialists", desc: "Staff certified to cut trees without impacting structural tree health." },
                { icon: "Clock", title: "Fast Storm Response", desc: "Dispatched local crews clearing emergency blockages and hazards 24/7." },
                { icon: "ThumbsUp", title: "Complete Site Cleanup", desc: "We chip wood, grind stumps, and clean lawns to leave yards immaculate." }
              ].map((item, idx) => (
                <div key={idx} className="bg-slate-50 p-8 rounded-3xl border border-slate-150/70 shadow-sm flex flex-col justify-between hover:shadow-xl hover:-translate-y-1 transition-all duration-300 text-left">
                  <div>
                    <div className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center mb-6">
                      <ServiceIcon name={item.icon} className="h-6 w-6 text-emerald-700" />
                    </div>
                    <h4 className="font-extrabold text-slate-900 text-lg">{item.title}</h4>
        ...
        <section id="about" className="bg-slate-50 py-20 md:py-28 px-6 border-t border-slate-100">
          <div className="container mx-auto grid md:grid-cols-2 gap-12 items-center max-w-5xl">
            ...
            <div className="text-left">
              <span className="text-xs font-bold uppercase tracking-widest text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100">Local Specialists</span>
              <h2 className="text-3xl md:text-4xl font-black text-slate-900 mt-5 tracking-tight">
                {content.about.title}
              </h2>
              <div className="w-12 h-1 mt-3 rounded bg-emerald-600" />
              <p className="mt-6 text-slate-600 leading-relaxed text-base">
                {content.about.body}
              </p>
              <ul className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                {(content.about.points || [
                  "Certified Tree Care Pros",
                  "24/7 Emergency Storm Dispatch",
                  "Low-Impact Rigging Systems",
                  "Eco-Friendly Green Disposal"
        ...
        <section id="reviews" className="py-20 md:py-28 px-6 bg-white">
          <div className="container mx-auto text-center max-w-5xl">
            <span className="text-xs font-bold uppercase tracking-widest text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100">Customer Reviews</span>
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 mt-5 tracking-tight">
        ...
        {content.faqs && content.faqs.length > 0 && (
          <section id="faqs" className="py-20 md:py-28 px-6 bg-slate-50 border-t border-slate-100">
            <div className="container mx-auto max-w-3xl">
              <span className="text-xs font-bold uppercase tracking-widest text-emerald-700 text-center block">Helpful Information</span>
              ...
              <Accordion type="single" collapsible className="w-full space-y-4">
                {content.faqs.map((faq: any, i: number) => (
                  <AccordionItem key={i} value={`faq-${i}`} className="border border-slate-200 rounded-2xl px-6 bg-white">
                    <AccordionTrigger className="text-left font-bold text-slate-800 text-base md:text-lg hover:no-underline py-4 hover:text-emerald-700 transition-colors">
                      {faq.question}
                    </AccordionTrigger>
        ...
        <section id="contact" className="relative py-20 md:py-28 px-6 bg-emerald-50 border-t border-emerald-100">
          <div className="container mx-auto">
            <div className="bg-white p-8 md:p-12 rounded-[32px] shadow-2xl max-w-lg mx-auto border border-slate-100">
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="mt-8 space-y-4">
                <div>
                  <Input placeholder="Your Name" className="h-12 rounded-xl border-slate-200 bg-slate-50 focus-visible:ring-emerald-500" {...form.register('name')} />
                  {form.formState.errors.name && <p className="text-xs text-rose-500 mt-1 text-left">{form.formState.errors.name.message}</p>}
                </div>
                <div>
                  <Input type="email" placeholder="Email Address" className="h-12 rounded-xl border-slate-200 bg-slate-50 focus-visible:ring-emerald-500" {...form.register('email')} />
                  {form.formState.errors.email && <p className="text-xs text-rose-500 mt-1 text-left">{form.formState.errors.email.message}</p>}
                </div>
                <div>
                  <Input type="tel" placeholder="Phone Number" className="h-12 rounded-xl border-slate-200 bg-slate-50 focus-visible:ring-emerald-500" {...form.register('phone')} />
                  {form.formState.errors.phone && <p className="text-xs text-rose-500 mt-1 text-left">{form.formState.errors.phone.message}</p>}
                </div>
                <div>
                  <Textarea placeholder="Please describe tree services needed (e.g. Oak removal, Palm trimming, stump grinding)..." className="min-h-[100px] rounded-xl border-slate-200 bg-slate-50 focus-visible:ring-emerald-500" {...form.register('notes')} />
                ...
                <div className="flex items-start space-x-3 text-left mt-6 bg-slate-50 p-4 rounded-2xl border border-slate-150">
                  <Checkbox 
                    ...
                    className="mt-0.5 border-slate-350 text-emerald-600 focus:ring-emerald-500 rounded"
                  />
                  <div className="grid gap-1.5 leading-tight">
                    ...
                    <p className="text-[10px] text-slate-400 leading-normal">
                      By checking this box, you consent to receive SMS updates regarding your tree service estimate. Msg & data rates may apply. Reply STOP to opt-out. Read our <a href={`/api/legal/privacy?userId=${businessProfileId}`} target="_blank" className="underline hover:text-emerald-700">Privacy Policy</a>.
                    </p>
                  </div>
                </div>
                {form.formState.errors.consent && <p className="text-xs text-rose-500 mt-1 text-left">{form.formState.errors.consent.message}</p>}
                
                <Button type="submit" className="w-full h-12 font-bold text-white tracking-wide shadow-md transition-all duration-300 hover:scale-102 mt-6 rounded-xl bg-emerald-700 hover:bg-emerald-800" disabled={isSubmitting}>
