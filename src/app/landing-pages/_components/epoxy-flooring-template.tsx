...
import { 
  ...
  Calendar, ArrowRight, Hammer, Flame, LayoutGrid, ShieldAlert,
  Layers, HardHat, Sparkles, CheckCircle2 
} from 'lucide-react';
...
function ServiceIcon({ name, className }: { name?: string; className?: string }) {
  if (name === 'Wrench') return <HardHat className={className} />;
  if (name === 'Shield') return <ShieldCheck className={className} />;
  if (name === 'Palette') return <Sparkles className={className} />;
  if (name === 'Sun') return <Layers className={className} />;
...
export function EpoxyFlooringTemplate({
  businessProfileId,
  ...
  useEffect(() => {
    const staticContent = getContentForService("Epoxy Flooring");
    if (websiteConfig) {
      const mergedReviews = [
  ...
  const heroContent = (
    <div className="relative z-10 p-6 max-w-4xl text-left">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-8 max-w-lg">
        {["10-Year Anti-Peel Warranty", "Commercial & Residential Grade", "Dust-Free Diamond Grinding Prep", "Chemical & Hot Tire Resistant"].map((feat, idx) => (
      ...
      <div className="flex flex-wrap gap-4 mt-10">
        <a href="#contact">
          <Button size="lg" className="bg-blue-600 hover:bg-blue-700 text-white font-bold h-14 px-8 rounded-xl transition-all hover:scale-105 shadow-lg shadow-blue-600/20">
            {content.hero.ctaText || "GET MY FREE ESTIMATE"}
          </Button>
        </a>
        <a href={`tel:${phone}`}>
  ...
  return (
    <div className="bg-white text-slate-900 font-sans antialiased">
      ...
      <div className="bg-slate-900 text-slate-100 py-2.5 px-6 text-center text-xs font-semibold flex justify-center items-center gap-4">
        <span>💎 Industrial Grade Epoxy & Polyaspartic Coatings</span>
        ...
        <a href={`tel:${phone}`} className="flex items-center gap-1 hover:underline">
          <Phone className="h-3.5 w-3.5" /> Call Local Installers: {phone}
        </a>
      </div>
      ...
      <header className="sticky top-0 z-50 py-4 px-6 md:px-12 flex justify-between items-center border-b border-slate-100 bg-white/95 backdrop-blur-md">
        ...
        <nav className="hidden md:flex gap-8 items-center text-sm font-bold text-slate-600">
          ...
          <a href="#process" className="hover:text-blue-600 transition-colors">Prep Process</a>
          ...
          <a href={blogLink} className="hover:text-blue-600 transition-colors">Blog</a>
        </nav>
      ...
      <main>
          ...
          <div className="container mx-auto grid lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-5 relative aspect-[4/3] md:aspect-square w-full rounded-[32px] overflow-hidden shadow-2xl border-4 border-white">
              {heroImages && heroImages[0] ? (
                <Image src={heroImages[0].imageUrl} alt="Sleek Epoxy Floor" fill className="object-cover" priority />
              ) : (
                <div className="absolute inset-0 bg-blue-100 flex items-center justify-center"><Layers className="h-20 w-20 text-blue-650" /></div>
              )}
            </div>
          </div>
        </section>
        ...
        <section className="bg-slate-50 border-y border-slate-100 py-12">
          <div className="container mx-auto px-6 max-w-5xl">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
              {[
                { icon: ShieldCheck, title: "10-Year Coating Warranty", desc: "Our multi-layer epoxy formulas are fully protected against hot tire peeling." },
                { icon: Award, title: "Advanced Concrete Prep", desc: "We use professional diamond grinding machinery to ensure maximum bonding." },
                { icon: ThumbsUp, title: "Owner-Supervised Quality", desc: "Every floor is double-inspected for a perfect, seamless, bubble-free cure." }
              ].map((item, idx) => (
        ...
        <section id="services" className="py-20 md:py-28 px-6 bg-white">
          <div className="container mx-auto text-center max-w-5xl">
            <span className="text-xs font-bold uppercase tracking-widest text-blue-750 bg-blue-50 px-3 py-1 rounded-full border border-blue-100">Coating Solutions</span>
                ...
                <div key={i} className="bg-slate-50 p-8 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-between hover:shadow-xl hover:-translate-y-2 transition-all duration-300 text-left group">
                  <div>
                    <div className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center mb-6 transition-transform duration-300 group-hover:scale-110">
                      <ServiceIcon name={item.iconName || item.icon} className="h-6 w-6 text-blue-600" />
                    </div>
        ...
        {content.process && (
          <section id="process" className="py-20 md:py-28 px-6 bg-slate-50 border-y border-slate-100">
            <div className="container mx-auto text-center max-w-5xl">
              <span className="text-xs font-bold uppercase tracking-widest text-blue-700 bg-blue-50 px-3 py-1 rounded-full border border-blue-100">Installation Sequence</span>
              <h2 className="text-3xl md:text-5xl font-black text-slate-900 mt-5 tracking-tight">
                {content.process.title || "How It Works"}
              </h2>
              <div className="w-16 h-1 mx-auto mt-4 rounded bg-blue-600" />
              <p className="text-slate-500 mt-4 max-w-xl mx-auto text-base">
                {content.process.subtitle || "The multi-stage process we use to guarantee a perfect bond."}
              </p>
        ...
        <section className="bg-white py-20 md:py-28 px-6">
          <div className="container mx-auto text-center max-w-5xl">
            <span className="text-xs font-bold uppercase tracking-widest text-blue-700 bg-blue-50 px-3 py-1 rounded-full border border-blue-100">Our Advantages</span>
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 mt-5 tracking-tight">
              The Professional Coating Difference
            </h2>
            <div className="w-16 h-1 mx-auto mt-4 rounded bg-blue-600" />
            <p className="text-slate-500 mt-4 max-w-xl mx-auto text-base">
              Why Tampa home and business owners trust our technicians for premium flooring finishes.
            </p>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mt-16">
              {[
                { icon: "ShieldAlert", title: "Commercial Adhesion", desc: "Meticulous diamond grinding prep work ensures resins penetrate concrete pores deep." },
                { icon: "Layers", title: "Polyaspartic Topcoats", desc: "Sealed with professional aliphatic polyaspartic coats that never amber or yellow." },
                { icon: "Clock", title: "Minimal Downtime", desc: "Rapid-cure options allow homeowners to return items and park cars in 48-72 hours." },
                { icon: "ThumbsUp", title: "Vast Flake Options", desc: "Select from metallic pigments, quartz blends, or over 15 decorative flake patterns." }
              ].map((item, idx) => (
                <div key={idx} className="bg-slate-50 p-8 rounded-3xl border border-slate-150/70 shadow-sm flex flex-col justify-between hover:shadow-xl hover:-translate-y-1 transition-all duration-300 text-left">
                  <div>
                    <div className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center mb-6">
                      <ServiceIcon name={item.icon} className="h-6 w-6 text-blue-600" />
                    </div>
                    <h4 className="font-extrabold text-slate-900 text-lg">{item.title}</h4>
        ...
        <section id="about" className="bg-slate-50 py-20 md:py-28 px-6 border-t border-slate-100">
          <div className="container mx-auto grid md:grid-cols-2 gap-12 items-center max-w-5xl">
            ...
            <div className="text-left">
              <span className="text-xs font-bold uppercase tracking-widest text-blue-700 bg-blue-50 px-3 py-1 rounded-full border border-blue-100">Local Flooring Experts</span>
              ...
              <ul className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                {(content.about.points || [
                  "10-Year Flake Coating Warranty",
              ...
              <Accordion type="single" collapsible className="w-full space-y-4">
                {content.faqs.map((faq: any, i: number) => (
                  <AccordionItem key={i} value={`faq-${i}`} className="border border-slate-200 rounded-2xl px-6 bg-white">
                    <AccordionTrigger className="text-left font-bold text-slate-800 text-base md:text-lg hover:no-underline py-4 hover:text-blue-600 transition-colors">
                      {faq.question}
                    </AccordionTrigger>
          ...
          <div className="container mx-auto">
            <div className="bg-white p-8 md:p-12 rounded-[32px] shadow-2xl max-w-lg mx-auto border border-slate-100">
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="mt-8 space-y-4">
                ...
                <div>
                  <Textarea placeholder="Please describe flooring needs (e.g. 2-car garage flake, commercial warehouse, patio metallic)..." className="min-h-[100px] rounded-xl border-slate-200 bg-slate-50 focus-visible:ring-blue-500" {...form.register('notes')} />
                ...
                <div className="flex items-start space-x-3 text-left mt-6 bg-slate-50 p-4 rounded-2xl border border-slate-150">
                  ...
                  <div className="grid gap-1.5 leading-tight">
                    ...
                    <p className="text-[10px] text-slate-400 leading-normal">
                      By checking this box, you consent to receive SMS updates regarding your flooring estimate. Msg & data rates may apply. Reply STOP to opt-out. Read our <a href={`/api/legal/privacy?userId=${businessProfileId}`} target="_blank" className="underline hover:text-blue-700">Privacy Policy</a>.
                    </p>
                  </div>
                </div>
                {form.formState.errors.consent && <p className="text-xs text-rose-500 mt-1 text-left">{form.formState.errors.consent.message}</p>}
                
                <Button type="submit" className="w-full h-12 font-bold text-white tracking-wide shadow-md transition-all duration-300 hover:scale-102 mt-6 rounded-xl bg-blue-600 hover:bg-blue-700" disabled={isSubmitting}>
