            ...
            {quizFinished ? (
              <>
                <div className="p-6 md:p-8">
                  <CardHeader className="text-center">
                    <CardTitle className="text-2xl md:text-3xl"><TranslatedText>Quiz Complete!</TranslatedText></CardTitle>
                    <CardDescription><TranslatedText>Here are your results.</TranslatedText></CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="text-center">
                      ...
                      <p className="text-base md:text-lg text-muted-foreground mb-8 min-h-[56px]">
                        <TranslatedText>{getResultMessage()}</TranslatedText>
                      </p>
                    </div>
                    ...
                    <div className="mt-8 border-t pt-8">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                          <div className="text-left">
                              <h3 className="text-xl font-semibold mb-4 text-center"><TranslatedText>Key Insights</TranslatedText></h3>
                              <ul className="space-y-3 text-muted-foreground">
                                  <li className="flex items-start gap-3">
                                      <CheckCircle className="h-5 w-5 text-primary mt-1 flex-shrink-0" />
                                      <span><TranslatedText>Based on your score, you have a solid foundation but could see significant growth by automating lead follow-up.</TranslatedText></span>
                                  </li>
                                  <li className="flex items-start gap-3">
                                      <CheckCircle className="h-5 w-5 text-primary mt-1 flex-shrink-0" />
                                      <span><TranslatedText>Your answers suggest that your team spends valuable time on repetitive manual tasks that could be automated.</TranslatedText></span>
                                  </li>
                                  <li className="flex items-start gap-3">
                                      <CheckCircle className="h-5 w-5 text-primary mt-1 flex-shrink-0" />
                                      <span><TranslatedText>There's a clear opportunity to implement a 24/7 AI-driven system to capture and qualify leads, ensuring you never miss an opportunity.</TranslatedText></span>
                                  </li>
                              </ul>
                          </div>
                          ...
                          <div className="space-y-8">
                              <div>
                                  <h3 className="text-xl font-semibold mb-4 text-center"><TranslatedText>Recommended Next Steps</TranslatedText></h3>
                                   <div className="text-center bg-muted/50 p-6 rounded-lg space-y-4">
                                      <div>
                                        <p className="text-muted-foreground mb-4">
                                            <TranslatedText>Ready to turn these insights into action? Schedule a free, no-obligation consultation with one of our automation experts.</TranslatedText>
                                        </p>
                                        <Link href="/contact">
                                            <Button size="lg"><TranslatedText>Book a Free Consultation</TranslatedText></Button>
                                        </Link>
                                      </div>
                                      <div className="border-t pt-4">
                                        <p className="text-muted-foreground mb-4">
                                            <TranslatedText>Or view your personalized plan.</TranslatedText>
                                        </p>
                                        <Link href={getPlanUrl()}>
                                            <Button size="lg" variant="outline">
                                              <TranslatedText>View Your Custom Plan</TranslatedText>
                                            </Button>
                                        </Link>
                                      </div>
                                  </div>
              ...
              <>
                <CardHeader className="p-4 md:p-6">
                  <CardTitle className="text-xl md:text-2xl"><TranslatedText>Free Assessment Quiz</TranslatedText></CardTitle>
                  <CardDescription><TranslatedText>See how your customer acquisition strategy stacks up.</TranslatedText></CardDescription>
                  <div className="pt-4">
                ...
                <CardContent className="text-center py-8 px-4 md:px-6">
                  <div className="min-h-[120px] md:min-h-[84px] flex flex-col items-center justify-center mb-8">
                    <p className="text-lg md:text-xl font-semibold">
                      <TranslatedText>{currentQuestion.question}</TranslatedText>
                    </p>
                  </div>
                  
                  {currentQuestion.type === 'yes-no' && (
                    <div className="flex justify-center gap-4">
                      <Button onClick={() => handleAnswer('yes')} className="w-28 md:w-32 h-12 text-lg bg-[#1A1A1A] hover:bg-[#C6410F]"><TranslatedText>Yes</TranslatedText></Button>
                      <Button onClick={() => handleAnswer('no')} className="w-28 md:w-32 h-12 text-lg bg-[#1A1A1A] hover:bg-[#C6410F]"><TranslatedText>No</TranslatedText></Button>
                    </div>
                  )}

                  {currentQuestion.type === 'multiple-choice-single' && (
                    <div className="space-y-2 text-left max-w-lg mx-auto">
                        <p className="text-muted-foreground text-sm mb-2 text-left"><TranslatedText>(Choose one)</TranslatedText></p>
                        <RadioGroup onValueChange={handleMultipleChoiceSingleChange} value={answers[currentQuestionIndex] as string}>
                            {currentQuestion.options?.map(option => (
                                <div key={option} className="flex items-center space-x-2 p-3 rounded-md border hover:bg-muted transition-colors">
                                    ...
                                    <Label htmlFor={option} className="flex-1 cursor-pointer"><TranslatedText>{option}</TranslatedText></Label>
                                </div>
                            ))}
                        </RadioGroup>
                        <Button onClick={handleNextQuestion} className="w-full mt-6" disabled={!answers[currentQuestionIndex]}><TranslatedText>Next</TranslatedText></Button>
                    </div>
                  )}

                  {currentQuestion.type === 'multiple-choice-multiple' && (
                      <div className="space-y-2 text-left max-w-lg mx-auto">
                          <p className="text-muted-foreground text-sm mb-2 text-left"><TranslatedText>(Choose all that apply)</TranslatedText></p>
                          {currentQuestion.options?.map(option => (
                              <div key={option} className="flex items-center space-x-2 p-3 rounded-md border hover:bg-muted transition-colors">
                                  <Checkbox 
                                      id={option} 
                          ...
                          <Button onClick={handleNextQuestion} className="w-full mt-6"><TranslatedText>Next</TranslatedText></Button>
                      </div>
                  )}
                  ...
                  {currentQuestion.type === 'text' && (
                      <div className="space-y-4 max-w-lg mx-auto">
                          ...
                          />
                          <Button onClick={handleNextQuestion} className="w-full mt-4"><TranslatedText>Next</TranslatedText></Button>
                      </div>
                  )}
