...
(async () => {
  const bot = await createDevBot({
    ...
    workspaceRoot: process.env.WORKSPACE_ROOT || "/Volumes/SAMSUNG 500gb/Antigravity/T3 Automations",
    workspaceName: "T3 Automations",
    firebaseProjectId: "studio-1410114603-9e1f6",
  });

  console.log("⚡ T3 Automations Dev Bot starting...");

  async function startWithRetry(maxRetries = 5, delayMs = 5000) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        await bot.start({
          onStart: () => console.log("✅ @T3AutomationsDevBot is running"),
        });
        return;
      } catch (err: any) {
